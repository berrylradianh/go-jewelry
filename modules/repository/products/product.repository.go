...
func (productRepo *Repository) GetAllProducts() (*[]ep.Product, error) {
	...
	if err := productRepo.DB.Preload("Transaction_detail", "deleted_at IS NULL").Find(&products).Error; err != nil {
		return nil, err
	}
...
func (productRepo *Repository) GetProductById(id int) (*ep.Product, error) {
	...
	if err := productRepo.DB.Preload("Transaction_detail", "deleted_at IS NULL").First(&product, id).Error; err != nil {
		return nil, err
	}
